...
int main(void) {
  ...
  uint8_t addr[5] = {0xE7, 0xE7, 0xE7, 0xE7,
                     0xE7}; // must match the receiver's RX_ADDR_P0

  nrf24_write_addr_reg(NRF24_REG_TX_ADDR, addr, 5);
  nrf24_write_addr_reg(NRF24_REG_RX_ADDR_P0, addr,
                       5); // needed so this node can receive the ACK
  nrf24_write_reg(NRF24_REG_RF_CH,
                  76); // 2400 + 76 = 2476 MHz; must match the receiver
  nrf24_write_reg(NRF24_REG_CONFIG,
                  0x0A); // EN_CRC=1, CRCO=0, PWR_UP=1, PRIM_RX=0
